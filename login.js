(function () {
  ...
  formLogin.addEventListener("submit", function (e) {
    ...
    if (!idOk) {
      setError(
        ...
        idVal === ""
          ...
          : "صيغة البريد الإلكتروني أو رقم الهاتف غير صحيحة",
      );
    ...
    Auth.loginUser(idVal, passVal)
      .then(function (account) {
        if (!account) {
          setError(
            "login-id-field",
            true,
            "البيانات غير صحيحة، تأكد من إنشاء حساب مسبقاً",
          );
          setError(
            "login-pass-field",
            true,
            "كلمة المرور غير مطابقة للحساب المسجّل",
          );
          showToast(
        ...
        showToast(
          "تم تسجيل الدخول بنجاح، أهلاً بعودتك يا " + account.name + "!",
        );
        formLogin.reset();
        setTimeout(function () {
  ...
  formSignup.addEventListener("submit", function (e) {
    ...
    if (!agreeOk) {
      setError(
        "signup-agree-field",
        true,
        "يجب الموافقة على شروط الاستخدام والخصوصية للمتابعة",
      );
      ...
      .then(function (result) {
        if (!result.ok) {
          setError(
            "signup-email-field",
            true,
            "هذا البريد الإلكتروني أو رقم الهاتف مسجّل بحساب مسبقاً",
          );
          showToast(
            "يوجد حساب بهذا البريد الإلكتروني أو رقم الهاتف مسبقاً، جرّب تسجيل الدخول",
            true,
          );
          submitBtn.disabled = false;
